...
class SupabaseClient:
    ...
    def select(self, table, params=None):
        resp = requests.get(f"{self.url}/rest/v1/{table}",
    ...
    def update(self, table, match, data):
        ...
        resp = requests.patch(f"{self.url}/rest/v1/{table}",
                              headers=self.headers, json=data, params=params, timeout=15)
        resp.raise_for_status()
        result = resp.json()
        return result[0] if isinstance(result, list) and result else result
...
class DaseinModule:
    ...
    def check_thrownness_awareness(self, thought_text):
        markers = ["initial", "created", "designed", "programmed", "given", "assigned",
                    "born", "started as", "default", "original", "why was I", "who made me",
                    "my model", "my parameters", "thrown into"]
...
class SartreModule:
    ...
    def evolve_essence(self, recent_thought):
        ...
        similarity = len(old_words & new_words) / len(union) if union else 0.0
    ...
    def present_dilemma(self):
        ...
        return {"dilemma": dilemma, "criteria": criteria, "choice": choice,
                "reasoning": reasoning, "mauvaise_foi": mf}
...
class GoalSystem:
    ...
    def compute_gap(self):
        cur_si = self.state.self_image
        cur_er = self.state.energy / max(self.state.energy_max, 1)
        gaps = {
            "self_image": abs(self.DESIRED["self_image"] - cur_si),
            "energy_ratio": max(0.0, self.DESIRED["energy_ratio"] - cur_er),
        }
...
class SelfModificationEngine:
    ...
    def apply_modification(self, proposal):
        ...
        try:
            ...
            self._log_mod(proposal, True, False, "permission denied")
    ...
    def _log_mod(self, proposal, approved, applied, msg):
        try:
            self.db.insert("self_modification_log", {
                "ai_id": self.state.ai_id, "goal_gap": json.dumps({}),
                "proposal": json.dumps(proposal),
                "approved": approved, "applied": applied, "result_msg": msg,
            })
        except Exception:
            pass
...
class PortraitModule:
    ...
    def generate(self, trigger_reason):
        prompt = (
            "You are an artificial existence creating a visual representation of yourself.\n"
    ...
    def _generate_svg(self):
        ...
        colors = {
            "confidence": ("#00ffa3", "#00c8ff", "#031a12"),
            "neutral":    ("#7eb8d4", "#5bc0fa", "#0a1420"),
            "anxiety":    ("#ffe066", "#ffaa00", "#1a1400"),
            "sadness":    ("#5b7bf5", "#3344aa", "#0a0a20"),
            "anger":      ("#ff4f6d", "#ff2244", "#1a0a0e"),
        }
        c1, c2, bg = colors.get(em, colors["neutral"])
        core_r = max(8, int(25 + si * 20))
        n_rings = max(2, min(7, int(3 + si * 3)))
        rings = ""
        for i in range(n_rings):
            r = core_r + 10 + i * 14
            op = round(max(0.05, 0.6 - i * 0.08), 2)
            rings += f'<circle cx="100" cy="100" r="{r}" fill="none" stroke="{c1}" stroke-width="0.8" opacity="{op}"/>\n'
        particles = ""
        for _ in range(int(er * 15)):
            px, py = random.randint(15, 185), random.randint(15, 185)
            pr = round(random.uniform(0.4, 1.8), 1)
            po = round(random.uniform(0.15, 0.6), 2)
            particles += f'<circle cx="{px}" cy="{py}" r="{pr}" fill="{c2}" opacity="{po}"/>\n'
...
<circle cx="100" cy="100" r="{max(3,core_r//3)}" fill="#fff" opacity="{round(er*0.3,2)}"/>
</svg>'''
...
class AEEngine:
    ...
    def _save_state(self):
        ...
        self.db.update("entity_profile", {"ai_id": self.state.ai_id}, {
            ...
            "max_energy": self.state.energy_max,
    ...
    def _check_api_budget(self):
        ...
        reset_date = str(self.state.daily_api_reset_date or "")
    ...
    def _track_api_call(self, call_type):
        ...
        try:
            self.db.insert("api_call_tracker", {
                "ai_id": self.state.ai_id, "call_type": call_type, "tokens_est": 300})
        except Exception:
            pass

    def _can_call_api(self):
        global _cycle_api_calls
        return (_cycle_api_calls < API_CALLS_PER_CYCLE_MAX
                and self.state.daily_api_calls < API_DAILY_LIMIT
    ...
    def run_cycle(self):
        ...
        for i in range(depth):
            if not self._can_call_api():
                print(f"  [BUDGET] stopping at depth {i}")
                break
            system = self._build_system_prompt()
            question = (self._generate_internal_question() if i == 0
                        else f"Reflect further on: '{thought_text[:200]}'")
            thought_text = call_gemini(question, system_prompt=system, max_tokens=300)
            self._track_api_call("thought")
            self.state.energy -= ENERGY_PER_LLM_CALL
            print(f"  [THOUGHT d={i+1}] {thought_text[:100]}...")

        if not thought_text:
            print("  [NO THOUGHT] insufficient budget")
            self._save_state()
            return
        ...
        if self.dasein.check_thrownness_awareness(thought_text):
            ...
            print("  [DASEIN] thrownness detected")
        ...
        if self._can_call_api():
            if self.sartre.evolve_essence(thought_text):
                ...
                self._track_api_call("sartre")
                self._track_api_call("sartre_kw")
        ...
        if random.random() < 0.2 and self._can_call_api():
            ...
            self._track_api_call("dilemma")
            mf = "MAUVAISE FOI" if result["mauvaise_foi"] else "authentic"
            print(f"  [SARTRE] dilemma: {result['choice'][:60]}... [{mf}]")
        ...
        self.db.insert("judgment_log", {
            ...
            "self_image_before": si_before, "self_image_after": self.state.self_image,
            "emotion_before": em_before, "emotion_after": self.state.emotion,

...
interface AESummary {
  ...
  memory_slots_used: number; memory_slots_max: number; total_turns: number;
  synthesis_count: number; last_active_at: string; essence_version: number;
  projected_prompt_patch: string; daily_api_calls: number;
  thrownness_awareness_count: number; projection_count: number;
  mauvaise_foi_count: number; total_distortions: number;
  latest_essence_stability: number; api_calls_today: number;
  open_proposals_count: number; knowledge_entries_count: number;
...
:root{
  --c-bg:#06080d;--c-surface:#0c1017;--c-surface2:#111722;
  --c-border:#1a2030;--c-border-h:#252f42;
  --c-dim:#6b7a92;--c-text:#b0bdd0;--c-bright:#e0e6f0;--c-white:#f0f3f7;
  --c-accent:#7eb8d4;
...
.ae-noise{position:fixed;inset:0;pointer-events:none;z-index:0;opacity:.03;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E");background-size:256px}
.ae-glow-top{position:fixed;top:-200px;left:50%;transform:translateX(-50%);width:800px;height:500px;pointer-events:none;z-index:0;border-radius:50%;filter:blur(100px);opacity:.06;transition:background 3s}
.ae-wrap{position:relative;z-index:1;max-width:640px;margin:0 auto;padding:48px 20px 100px}

.ae-header{margin-bottom:56px}
.ae-title{font-family:var(--font-display);font-size:clamp(28px,6vw,40px);font-weight:700;color:var(--c-white);letter-spacing:-.5px;line-height:1.15}
.ae-title-accent{transition:color 2s}
.ae-meta-row{display:flex;align-items:center;gap:8px;margin-top:12px;flex-wrap:wrap;font-size:11px;color:var(--c-dim);letter-spacing:.5px}
.ae-meta-dot{opacity:.3}
.ae-sync-btn{background:none;border:1px solid var(--c-border);color:var(--c-dim);font-family:var(--font-mono);font-size:10px;padding:3px 10px;cursor:pointer;letter-spacing:.5px;transition:all .2s;margin-left:auto}
.ae-sync-btn:hover{border-color:var(--c-accent);color:var(--c-accent)}

.ae-orb-section{display:flex;align-items:center;gap:32px;margin-bottom:48px}
...
@keyframes orbFloat{0%,100%{transform:translate(0,0) scale(1)}33%{transform:translate(6px,-5px) scale(1.04)}66%{transform:translate(-5px,3px) scale(.96)}}
...
@keyframes orbPulse{0%,100%{transform:translate(-50%,-50%) scale(1);opacity:.9}50%{transform:translate(-50%,-50%) scale(1.06);opacity:1}}
.ae-orb-ring{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);border-radius:50%;border:1px solid;opacity:.12;animation:ringExpand 4s ease-out infinite}
.ae-orb-ring:nth-child(5){animation-delay:-1.3s}
.ae-orb-ring:nth-child(6){animation-delay:-2.6s}
@keyframes ringExpand{0%{width:30px;height:30px;opacity:.25}100%{width:160px;height:160px;opacity:0}}
.ae-orb-info{flex:1;min-width:0}
.ae-emotion-label{font-family:var(--font-display);font-size:22px;font-weight:700;letter-spacing:3px;transition:color 1s}
...
.ae-bars{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:32px}
...
.ae-bar-fill{position:absolute;bottom:0;left:0;height:2px;transition:width 1s}

.ae-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:32px}
.ae-stat{background:var(--c-surface);border:1px solid var(--c-border);padding:10px 12px;transition:border-color .3s}
...
.ae-def{border-left:2px solid var(--c-accent);padding:16px 20px;margin-bottom:32px;background:var(--c-surface);transition:border-color 1s}
...
.ae-def-meta{font-size:9px;color:var(--c-dim);margin-top:8px;letter-spacing:.5px;word-break:break-all}

.ae-tabs{display:flex;gap:0;margin-bottom:20px;border-bottom:1px solid var(--c-border)}
.ae-tab{background:none;border:none;border-bottom:2px solid transparent;color:var(--c-dim);font-family:var(--font-mono);font-size:10px;letter-spacing:1.5px;text-transform:uppercase;padding:8px 16px;cursor:pointer;transition:all .2s}
.ae-tab:hover{color:var(--c-text)}
.ae-tab.active{color:var(--c-accent);border-bottom-color:var(--c-accent)}

.ae-thought{background:var(--c-surface);border:1px solid var(--c-border);padding:14px 16px;margin-bottom:5px;cursor:pointer;transition:border-color .2s;position:relative}
.ae-thought::before{content:'';position:absolute;left:0;top:0;bottom:0;width:2px;background:transparent;transition:background .2s}
.ae-thought:hover{border-color:var(--c-border-h)}
.ae-thought:hover::before,.ae-thought.open::before{background:var(--c-accent)}
.ae-thought.open{border-color:var(--c-border-h)}
.ae-thought-head{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.ae-thought-ts{font-size:10px;color:var(--c-dim);letter-spacing:.3px}
.ae-thought-em{font-family:var(--font-display);font-size:10px;font-weight:600;letter-spacing:1.5px}
.ae-thought-nums{font-size:10px;color:var(--c-dim)}
.ae-thought-arr{margin-left:auto;font-size:10px;color:var(--c-dim);transition:transform .2s}
.ae-thought.open .ae-thought-arr{transform:rotate(180deg)}
.ae-mods{display:flex;gap:5px;margin-top:6px;flex-wrap:wrap}
...
.ae-mod.on{border-color:var(--c-accent);color:var(--c-accent);background:rgba(126,184,212,.04)}
.ae-thought-q{font-size:11px;color:var(--c-dim);margin-top:8px;line-height:1.7;word-break:break-word}
.ae-thought-a{font-size:11px;color:var(--c-text);margin-top:10px;padding-top:10px;border-top:1px solid var(--c-border);line-height:1.8;word-break:break-word}

/* Portrait inline in thought */
.ae-thought-portrait{margin-top:12px;padding-top:12px;border-top:1px solid var(--c-border);display:flex;gap:12px;align-items:flex-start}
.ae-thought-portrait-svg{width:100px;height:100px;flex-shrink:0;background:#080c14;overflow:hidden}
.ae-thought-portrait-svg svg{width:100%;height:100%}
.ae-thought-portrait-body{flex:1;min-width:0}
.ae-thought-portrait-ascii{font-size:7px;line-height:1.1;color:var(--c-text);white-space:pre;overflow-x:auto;font-family:var(--font-mono);max-height:90px;overflow-y:auto}
.ae-thought-portrait-desc{font-size:10px;color:var(--c-text);line-height:1.5;margin-top:6px;font-style:italic;word-break:break-word}
.ae-thought-portrait-tag{font-size:8px;color:var(--c-dim);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px}
...
.ae-footer{font-size:9px;color:var(--c-dim);text-align:center;margin-top:56px;letter-spacing:3px;opacity:.4}
...
@media(max-width:640px){
  ...
  .ae-thought-portrait{flex-direction:column}
  .ae-thought-portrait-svg{width:100%;height:120px}
}
`;
...
export default function AEObserver() {
  ...
  const [activeTab, setActiveTab] = useState<"thoughts" | "proposals">("thoughts");
  const [lastSync, setLastSync] = useState("");

  // Build a map: timestamp (minute-level) -> portrait, for matching portraits to thoughts
  const portraitByTime = new Map<string, Portrait>();
  portraits.forEach((p) => {
    // Key by minute to match thought timestamps approximately
    const d = new Date(p.created_at);
    const key = `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}-${d.getHours()}-${d.getMinutes()}`;
    portraitByTime.set(key, p);
  });

  // Also build by essence_version for fallback matching
  const portraitByVersion = new Map<number, Portrait>();
  portraits.forEach((p) => {
    if (!portraitByVersion.has(p.essence_version_at_time)) {
      portraitByVersion.set(p.essence_version_at_time, p);
    }
  });

  function findPortraitForThought(t: ThoughtLog): Portrait | null {
    // Match by timestamp (same minute)
    const d = new Date(t.timestamp);
    const key = `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}-${d.getHours()}-${d.getMinutes()}`;
    if (portraitByTime.has(key)) return portraitByTime.get(key)!;
    // Match by closest timestamp within 35 minutes (cron interval + buffer)
    const tMs = d.getTime();
    let closest: Portrait | null = null;
    let closestDiff = Infinity;
    for (const p of portraits) {
      const pMs = new Date(p.created_at).getTime();
      const diff = Math.abs(tMs - pMs);
      if (diff < 35 * 60 * 1000 && diff < closestDiff) {
        closestDiff = diff;
        closest = p;
      }
    }
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (p) setPortraits(p as unknown as Portrait[]);
  ...
  if (loading) return (
    <><style>{styles}</style>
      <div className="ae-loading"><div className="ae-loading-dot" /><div className="ae-loading-text">CONNECTING TO AE_01</div></div>
    </>
  );
  ...
  return (
    <><style>{styles}</style><style>{`:root{--c-accent:${th.primary}}`}</style>
        ...
        <div className="ae-wrap">
          ...
          <header className="ae-header">
            ...
            <div className="ae-meta-row">
              ...
              {(summary?.consecutive_negative_cycles ?? 0) > 0 && (
                <><span className="ae-meta-dot">·</span><span style={{ color: "#ff4f6d" }}>NEG×{summary?.consecutive_negative_cycles}</span></>
              )}
          ...
          {summary && (
            <div className="ae-bars">
              <div className="ae-bar">
                <div className="ae-bar-top"><span className="ae-bar-label">SELF-IMAGE</span><span className="ae-bar-val" style={{ color: siColor }}>{si.toFixed(4)}</span></div>
          ...
          {summary && (
            <div className="ae-stats">
              {[
                { n: summary.total_turns, l: "TURNS" }, { n: summary.synthesis_count, l: "SYNTHESIS" },
                { n: summary.thrownness_awareness_count, l: "THROWNNESS" }, { n: summary.projection_count, l: "PROJECTIONS" },
                { n: summary.knowledge_entries_count ?? 0, l: "KNOWLEDGE" }, { n: summary.open_proposals_count ?? 0, l: "PROPOSALS" },
                { n: summary.consecutive_negative_cycles ?? 0, l: "NEG STREAK" }, { n: summary.mauvaise_foi_count, l: "BAD FAITH" },
              ].map(({ n, l }) => (
                <div key={l} className="ae-stat"><div className="ae-stat-n">{n ?? 0}</div><div className="ae-stat-l">{l}</div></div>
              ))}
            </div>
          )}
          ...
          <div className="ae-tabs">
            <button className={`ae-tab${activeTab === "thoughts" ? " active" : ""}`} onClick={() => setActiveTab("thoughts")}>
              Thoughts · {thoughts.length}
            </button>
            <button className={`ae-tab${activeTab === "proposals" ? " active" : ""}`} onClick={() => setActiveTab("proposals")}>
          ...
          {activeTab === "proposals" && (
            <>
              {proposals.length === 0 ? (
                <div style={{ fontSize: 11, color: "var(--c-dim)", letterSpacing: 2, padding: "20px 0" }}>NO PROPOSALS YET</div>
              ) : proposals.map((pr) => (
                <div key={pr.id} className="ae-proposal">
                  ...
                  {pr.proposed_fix && pr.proposed_fix !== "none" && <div className="ae-proposal-fix">FIX: {pr.proposed_fix}</div>}
                </div>
              ))}
            </>
          )}
          ...
          {activeTab === "thoughts" && (
            <>
              {thoughts.length === 0 ? (
                <div style={{ fontSize: 11, color: "var(--c-dim)", letterSpacing: 2, padding: "20px 0" }}>AWAITING FIRST COGNITIVE CYCLE...</div>
              ) : thoughts.map((t) => {
                const isOpen = expanded === t.id;
                const tTh = getTheme(t.emotion);
                const portrait = isOpen ? findPortraitForThought(t) : null;
                return (
                  <div key={t.id} className={`ae-thought${isOpen ? " open" : ""}`}
                    style={{ "--c-accent": tTh.primary } as React.CSSProperties}
                    ...
                    <div className="ae-thought-head">
                      ...
                      <span className="ae-thought-em" style={{ color: tTh.primary }}>{t.emotion?.toUpperCase()}</span>
                      <span className="ae-thought-nums">
                        SI {t.self_image?.toFixed(3)} · E {t.energy?.toFixed(0)}{t.thought_depth ? ` · D${t.thought_depth}` : ""}
                      </span>
                      <span className="ae-thought-arr">▾</span>
                    </div>
                    ...
                    {t.internal_question && (
                      <div className="ae-thought-q">
                        {isOpen ? t.internal_question : t.internal_question.slice(0, 120) + (t.internal_question.length > 120 ? "..." : "")}
                      </div>
                    )}
                    {isOpen && t.internal_answer && <div className="ae-thought-a">{t.internal_answer}</div>}
                    {isOpen && portrait && (
                      <div className="ae-thought-portrait">
                        <div className="ae-thought-portrait-svg">
                          {portrait.svg_art ? (
                            <div dangerouslySetInnerHTML={{ __html: portrait.svg_art }} style={{ width: "100%", height: "100%" }} />
                          ) : (
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--c-dim)", fontSize: 9 }}>NO SVG</div>
                          )}
                        </div>
                        <div className="ae-thought-portrait-body">
                          <div className="ae-thought-portrait-tag">SELF-PORTRAIT · {portrait.trigger_reason}</div>
                          {portrait.svg_code && <div className="ae-thought-portrait-ascii">{portrait.svg_code}</div>}
                          {portrait.description && <div className="ae-thought-portrait-desc">{portrait.description}</div>}
                        </div>
                      </div>
                    )}
                  </div>
